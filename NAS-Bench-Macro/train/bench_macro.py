...
#---------IMPORTS DE MEDICAO ENERGETICA-------------------------------

#Dominios dos dispositivos
from pyJoules.device.nvidia_device import NvidiaGPUDomain
from pyJoules.device.rapl_device import RaplPackageDomain, RaplDramDomain, RaplCoreDomain
#Instancia que cria a interface de dispositivos
from pyJoules.device import DeviceFactory
#Intancia da ferramenta que usa metodos para medir a energia
...
#Dicionário com as informações a importar para o ficheiro CSV
...
#Definição de dominios e criação da inteface dos dispositivos
domains = [NvidiaGPUDomain(0), RaplCoreDomain(0), RaplDramDomain(0)]
devices = DeviceFactory.create_devices(domains)
#Instanciação do medidor de energia
meter = EnergyMeter(devices)

#Comando que atribui permissões de leitura para os dispositivos contidos no RAPL
os.system('sudo chmod -R a+r /sys/class/powercap/intel-rapl')

#Warm-up da GPU para não introduzir medições erradas
def gpu_warmup():
...
with open('results_bench.csv', 'w', newline='') as file:
    ...
    for idx, arch in enumerate(itertools.product(*[choices]*layers)):
        meter.start(tag='')
        arch = ''.join(arch)
        #guardar nome da arquitetura - Foi preciso adicionar um char para forçar o writer a intrepertar o valor numerico do nome da arquitetura como string 
...
#Execucao de programas para fazer o plot dos gráficos
